...
const Salaries = () => {
  ...
  const salaries = useMemo(() => {
    ...
    return salariesList.map((salary, index) => ({
      ...
      remainingAmount:
        salary.remainingAmount != null ? salary.remainingAmount : "N/A",
      startDate: salary.startDate
        ? new Date(salary.startDate).toLocaleDateString()
        : "N/A",
      endDate: salary.endDate
        ? new Date(salary.endDate).toLocaleDateString()
        : "N/A",
      petrolExpense:
        salary.petrolExpense != null ? salary.petrolExpense : "N/A",
      otherExpense: salary.otherExpense != null ? salary.otherExpense : "N/A",
      timestamp: salary.timestamp
        ? new Date(salary.timestamp).toLocaleString()
        : "N/A",
    }));
  }, [salariesList]);
  ...
  const handleCreateSalary = (salaryData) => {
    if (isEditMode && selectedSalary) {
      // ✏️ UPDATE
      const updatedList = salariesList.map((item) =>
  ...
  const formatted = salariesList.map((s) => ({
    Name: s.name,
    Phone: s.phoneNumber,
    Salary: s.salaryAmount,
    Paid: s.paidAmount,
    Remaining: s.remainingAmount,
    Petrol: s.petrolExpense,
    Other: s.otherExpense,
    StartDate: s.startDate,
    EndDate: s.endDate,
  }));

  exportToExcel(formatted, "Salaries");
};
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-green-600">
                    ₨{summaryData.totalPaid.toLocaleString()}
                  </p>
                </div>
        ...
        {summaryData && Object.keys(summaryData.employees).length > 0 && (
          <div className="bg-white rounded-2xl shadow-md p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(summaryData.employees).map(
                ([employeeName, data]) => (
                  <div
                    key={employeeName}
                    className="p-5 bg-gradient-to-br from-gray-50 to-purple-50 rounded-xl border border-purple-100 hover:shadow-md transition-all"
                  >
                    <div className="mb-3">
